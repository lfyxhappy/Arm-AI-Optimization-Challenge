...
When a full `2/4/6/8` pass becomes thermally confounded, use **Repeat selected thread (1 warm-up + 5)** for a cooled control series. It preserves the same immutable v3 JSON/CSV/HTML evidence but contains only the selected candidate. Every new session explicitly records `runMode` (`auto_tune` or `targeted_repeat`) and its candidate-thread list; the aggregate CSV upgrades the previous header without dropping legacy rows. A targeted series is valid only for a same-thread, same-build, same-model, same-input comparison; it does not replace a complete auto-tune stage or establish a new device-wide thread recommendation.
