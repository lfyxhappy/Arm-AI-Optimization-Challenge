...
- Each formal auto-tune or targeted-repeat stage is automatically archived as immutable JSON, CSV, and HTML in app-private storage. It includes privacy-preserving system/user prompt SHA-256 fingerprints, UTF-8 byte counts, maximum output tokens, explicit run mode, and candidate-thread list, so cross-stage comparisons can verify controlled inputs without exporting prompt text.
